...
def todos():
    category_id = request.args.get('category', type=int)
    show_completed = request.args.get('show_completed', type=bool)
    ...
    todos = query.order_by(Todo.priority.desc(), Todo.due_date.asc()).all()
    categories = Category.query.all()
    return render_template('todos.html', todos=todos, categories=categories, 
                         selected_category=category_id, show_completed=show_completed)
...
@app.route('/todo/<int:id>/edit', methods=['POST'])
@login_required
def edit_todo(id):
    todo = Todo.query.get_or_404(id)
    if todo.user_id != current_user.id:
...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('DATABASE_URL', f"sqlite:///{os.path.join(basedir, 'instance/notes.db')}")
...
def index():
    ...
    return render_template('index.html', notes=notes, search_query=search, selected_category=category_id)
...
def view_note(id):
    ...
    html_content = markdown2.markdown(note.content)
    return render_template('view_note.html', note=note, html_content=html_content)
...
def todos():
    category_id = request.args.get('category', type=int)
    show_completed = request.args.get('show_completed', default=False, type=bool)
    ...
    todos = query.order_by(Todo.priority.desc(), Todo.due_date.asc()).all()
    return render_template('todos.html', todos=todos, selected_category=category_id, show_completed=show_completed)

# ... [Additional todo routes would follow the same pattern as note routes] ...
@app.route('/todos')
@login_required
def todos():
    category_id = request.args.get('category', type=int)
